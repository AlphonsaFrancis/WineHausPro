...
const PaymentPage = () => {
  ...
  const [consumingWalletAmount, setConsumingWalletAmount] = useState(0);
  ...
  useEffect(() => {
    axios
      .get(`${config.BASE_URL}api/v1/auth/get-user-wallet/${userId}/`)
      .then((response) => {
  ...
  useEffect(() => {
    if (consumeUserWallet) {
      if (userWalletAmount >= total) {
        setTotalAmount(0);
        setPaymentMethod("");
        setConsumingWalletAmount(total);
      } else {
        setTotalAmount(total - userWalletAmount);
        setConsumingWalletAmount(total - userWalletAmount);
      }
    } else {
      setTotalAmount(total);
      setConsumingWalletAmount(0);
    }
  }, [consumeUserWallet, userWalletAmount, total]);

  console.log(
    "Total amount:",
    total - (consumeUserWallet ? userWalletAmount : 0)
  );
  ...
  const handleCODPayment = async () => {
    try {
      ...
      const orderResponse = await axios.post(
        `${config.BASE_URL}api/v1/orders/payments/create/`,
        {
          payment_method: "cod",
          amount: totalAmount,
          cart_id: cartId,
          paymentfrom_wallet: consumingWalletAmount,
          payment_from_cod: total - consumingWalletAmount,
          payment_from_online: 0,
        }
      );
      ...
      if (consumeUserWallet) {
        ...
        try {
          await axios.post(
            `${config.BASE_URL}api/v1/auth/deduct-from-wallet/${userId}/`,
            {
              amount: amount,
            }
          );
          ...
          toast.error(
            "Failed to deduct amount from wallet. Please check your wallet balance."
          );
        }
      }
    ...
    try {
      const response = await axios.post(
        `${config.BASE_URL}api/v1/orders/payments/create/`,
        {
          ...
          paymentfrom_wallet: consumingWalletAmount,
          payment_from_cod: 0,
          payment_from_online: total - consumingWalletAmount,
        }
      );
  ...
  const verifyPayment = async (paymentId, orderId) => {
    try {
      // Verify the payment
      const response = await axios.post(
        `${config.BASE_URL}api/v1/orders/payments/verify/`,
        {
      ...
      if (consumeUserWallet) {
        try {
          ...
          console.error("Wallet deduction error:", walletError);
          toast.error(
            "Wallet deduction failed. Please check your wallet balance."
          );
        }
      }
  ...
  const clearCartItems = async () => {
    try {
      ...
      await axios.delete(
        `${config.BASE_URL}api/v1/orders/cart-items/clear/?user_id=${userId}`
      );
      navigate("/userorder");
        ...
        {selectedAddress && total ? (
          <div className="payment-content">
            <p className="total-amount">
              Proceeding to payment for total amount:{" "}
          ...
          <p className="error-message">
            Unable to proceed with payment due to missing details. Please go
            back and select an address.
          </p>
        )}
      </div>
    </div>
